...
router.post('/register', async (req, res): Promise<any> => {
  try {
    ...
    const userExists = await db.select().from(users).where(eq(users.email, email)).execute();
...
router.get('/me', protect, async (req: AuthRequest, res): Promise<any> => {
  try {
    const user = await db.select().from(users).where(eq(users.id, req.user!.id)).execute();
    if (user.length > 0) {
